...
export default function App() {
  ...
  async function 분류시작(강행: boolean, 게이트값: GateOut | null) {
    ...
    // 끊겼을 때 **어디까지 갔는지** 함께 보내려고 로컬 변수로 따라간다.
    // 상태(단계들)를 쓰지 않는 이유 — catch 로 넘어온 시점의 단계들 은
    // 이 함수가 만들어질 때의 옛 값이라 비어 있다.
    //
    // **try 밖에 선언한다.** 안에서 선언하면 catch 에서 안 보인다.
    // 자바의 try 블록 스코프와 같다.
    let 마지막단계 = 0;
    const 시작시각 = Date.now();
    ...
    try {
      ...
      // 스트림이 결과 없이 끊긴 경우만 서버에 알린다. 4xx/5xx 는 서버가
      // 이미 알고 로그에 남겼으므로 다시 알릴 것이 없다.
      if (e instanceof api.StreamDropped) {
        api.끊김보고({
          desc: desc.trim(),
          단계: 마지막단계,
          경과: (Date.now() - 시작시각) / 1000,
          사유: e.message,
        });
      }
      오류처리(e);
  ...
  return (
    ...
    <>
      ...
      <footer className="쪽 바닥">
        {잔여 && (
          <p className="사용량">
            {/* 둘 다 오늘 몫이다. 예전에는 브라우저 몫만 누적이라
                "이 브라우저" 였는데, 2026-08-27 에 하루 단위로 바꿨다. */}
            오늘 남은 횟수 — 이 브라우저 <b>{잔여.세션남은}/{잔여.세션상한}</b>
            {" · "}전체 <b>{잔여.일일남은}/{잔여.일일상한}</b>
          </p>
        )}
      </footer>
    </>
