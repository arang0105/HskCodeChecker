...
@st.cache_resource
def DB_준비():
    """테이블이 없으면 만든다. cache_resource 라 앱당 1회만 돈다."""
    storage.init_db()
    return True
...
if 결과 and 결과["오류"]:
    ...
    # 확신도. **high 가 아니면 눈에 띄게 경고한다.**
    # 근거가 있다 — 시험 30건에서 medium·low 는 6건 전부 오답이었다.
    확신도 = 결과["확신도"]
    if 확신도 == "high":
        st.success("모델이 **확신도 high** 로 답했습니다. 그래도 아래 확인 사항은 보세요.")
    else:
        st.error(
            f"모델이 **확신도 {확신도 or '미상'}** 로 답했습니다. "
            "**이 답은 틀렸을 가능성이 높습니다.**  \n"
            "시험 30건에서 high 가 아닌 답 6건은 **전부 오답**이었습니다."
        )
...
with st.sidebar:
    ...
    st.markdown(
        "- 시험 문제 30건 중 **앞 6자리 17건(56.7%)**, "
        "**10자리 전체 15건(50.0%)** 을 맞혔습니다.\n"
