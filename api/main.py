...
@app.get("/api/health")
def 상태():
    """살아 있는지, 데이터가 올라와 있는지, **어느 커밋이 돌고 있는지.**

    주간 점검이 이걸 찌른다.

    **버전을 넣은 이유** — 2026-08-28 에 두 번, 푸시한 코드가 배포에 올라갔는지
    확인할 방법이 없었다. 밖에서 헬스를 30초마다 찔러 재기동을 잡으려 했는데
    9분 동안 한 번도 못 잡았다. Render 는 새 인스턴스를 띄우고 헬스체크가
    통과한 뒤에 교체해서 **밖에서는 빈틈이 안 보인다.** 배포는 정상이었고
    확인 방법이 틀렸던 것이다.

    RENDER_GIT_COMMIT 은 Render 가 빌드할 때 자동으로 넣어 준다. 빌드 설정을
    건드릴 필요가 없다. 로컬에는 없으므로 '로컬' 이 나온다 — 그것도 정보다.
    잘못된 주소를 찌르고 있는지 바로 갈린다.

    **앞 7자만 낸다.** 저장소가 공개라 전체 해시를 숨길 이유는 없지만,
    사람이 눈으로 대조할 길이면 충분하다(git log --oneline 과 같은 길이다).
    """
    벡터 = 자원.get("인덱스", (None, None))[0]
    return {
        "ok": True,
        "버전": os.getenv("RENDER_GIT_COMMIT", "")[:7] or "로컬",
